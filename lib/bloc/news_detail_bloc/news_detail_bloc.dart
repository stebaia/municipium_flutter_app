...
class NewsDetailBloc extends Bloc<NewsDetailEvent, NewsDetailState> {
  ...
  void fetchNewsDetail(int newsId) =>
      add(FetchNewsDetailEvent(newsId: newsId));

  FutureOr<void> _fetchNewsDetail(
    FetchNewsDetailEvent fetchNewsDetailEvent,
    Emitter<NewsDetailState> emit) async{
      emit(const FetchingNewsDetailState());
      try{
        final newsDetail = await newsRepository.getNewsDetail(fetchNewsDetailEvent.newsId);
