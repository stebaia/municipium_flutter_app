...
class PoiDetailDTO extends DTO with EquatableMixin{
	...
	PoiDetailDTO({this.mail, this.twitterLink, this.openDataLogo, this.type, this.youtubeLink, this.municipalityId, this.puntiContatto, this.instagramLink, this.id, this.pointOfInterestCategories, this.fax, this.breve, this.longitude, this.image, this.images, this.openingTimesUrl, this.circoscrizione, this.entry, this.nomeAlternativo, this.fullscreen, this.phone, this.identificativo, this.name, this.excerpt, this.galleriaVideo, this.servizi, this.latitude, this.facebookLink, this.description, this.tipiLuogo, this.idOpenData, this.modalitaAccesso, this.openDataIdentifier, this.cap, this.updatedAt, this.ulterioriInformazioni, this.galleryId, this.address, this.tripAdvisorLink, this.isAlwaysOpen, this.strutturaResponsabile, this.openingTimes, this.url, this.luoghiCollegati, this.municipalityName, this.mail2, this.quartiere, this.argomenti, this.webSite});

	PoiDetailDTO.fromJson(Map<String, dynamic> json) {
		mail = json['mail'];
		twitterLink = json['twitter_link'];
		openDataLogo = json['open_data_logo'];
		type = json['type'];
		youtubeLink = json['youtube_link'];
		municipalityId = json['municipality_id'];
		if (json['punti_contatto'] != null) {
			puntiContatto = <PuntiContatto>[];
			json['punti_contatto'].forEach((v) { puntiContatto!.add(new PuntiContatto.fromJson(v)); });
		}
		instagramLink = json['instagram_link'];
		id = json['id'];
		if (json['point_of_interest_categories'] != null) {
			pointOfInterestCategories = <PointOfInterestCategories>[];
			json['point_of_interest_categories'].forEach((v) { pointOfInterestCategories!.add(new PointOfInterestCategories.fromJson(v)); });
		}
		fax = json['fax'];
		breve = json['breve'];
	...
	Map<String, dynamic> toJson() {
      ...
      data['punti_contatto'] = this.puntiContatto!.map((v) => v.toJson()).toList();
    }
